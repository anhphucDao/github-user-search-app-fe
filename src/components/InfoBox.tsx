...
export default function InfoBox() {
    ...
    return (
        <div
            className={`flex flex-col w-[20.44rem] h-[32.31rem] sm:w-[35.81rem] sm:h-[30.06rem] md:w-[45.63rem] md:h-[27.75rem] rounded-[0.94rem] drop-shadow-lg ${
                themeConfig[theme as keyof typeof themeConfig].bgSecondary
            } mt-[1rem] sm:mt-[1.5rem] mb-[5rem] sm:mb-[14.75rem] md:mb-[9rem]`}
        >
            <div className="flex flex-row mt-[2rem] order-1 items-center md:mt-[3rem] ">
                ...
                <div className="flex flex-col grow ml-[1.25rem] justify-center md:grid md:grid-cols-2 md:self-start ">
                    <div
                        className={`text-[1.63rem] font-bold md:order-1 ${
                            theme === 'light' ? 'text-[#2B3442]' : 'text-[#fff]'
                        } `}
                    >{`${userData.name ? userData.name : ''}`}</div>
                    ...
                    <div
                        ...
                        } md:order-2 md:justify-self-start md:self-center`}
                    >
            ...
            <div
                className={`order-2 mx-auto text-left text-[0.75rem] ${
                    themeConfig[theme as keyof typeof themeConfig].text
                } mt-[2rem] w-[17.44rem] sm:w-[30.81rem] md:w-[30rem] h-[4.69rem] sm:h-[3.13rem] md:ml-[10rem] md:mt-[-2.5rem] md:py-0`}
            >
                {`${userData.bio ? userData.bio : 'This profile has no bio'}`}
            </div>
            <div
                className={`flex flex-row w-[17.44rem] h-[5.31rem] items-center order-3 ${
                    themeConfig[theme as keyof typeof themeConfig].bgPrimary
                }  ${
                    themeConfig[theme as keyof typeof themeConfig].text
                } mx-auto rounded-[0.63rem] justify-evenly text-[1rem] mt-[1.44rem] sm:w-[30.81rem] sm:h-[5.31rem]  md:ml-[10rem]`}
            >
                <div className={`flex flex-col place-items-center `}>
                    <div className=" text-[0.75rem] sm:text-[1rem]">Repos</div>
                    <div className={`font-bold ${theme === 'light' && 'text-github-gunMetal'} `}>
                        {userData.public_repos}
                    </div>
                </div>
                <div className="flex flex-col place-items-center">
                    <div className="text-[0.75rem] sm:text-[1rem]">Followers</div>
                    <div className={`font-bold ${theme === 'light' && 'text-github-gunMetal'} `}>
                        {userData.followers}
                    </div>
                </div>
                <div className="flex flex-col place-items-center">
                    <div className="text-[0.75rem] sm:text-[1rem]">Following</div>
                    <div className={`font-bold ${theme === 'light' && 'text-github-gunMetal'} `}>
                        {userData.following}
                    </div>
                </div>
            ...
            <div
                className={`flex flex-col order-4 mt-[1.5rem] w-[17.44rem] h-[7.94rem] ml-[1.5rem] text-[0.875rem] sm:text-[1rem] justify-between ${
                    themeConfig[theme as keyof typeof themeConfig].text
                } mb-[3rem] sm:grid sm:grid-cols-2 sm:ml-[2.5rem] sm:w-[26.81rem] sm:h-[3.94rem] sm:gap-x-[4.06rem] sm:gap-y-[1rem] md:ml-[10rem]`}
            >
                <div
